...
const App = () => {
  const [oldDevices] = useState([
    { id: 'samsung_antigo_frp', title: 'Samsung Antigo - Conta Google (FRP)', price: 'R$ 20,00', description: 'RemoÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o de Conta Google (FRP) em modelos Samsung mais antigos (J-series, A0x). ServiÃƒÆ’Ã‚Â§o rÃƒÆ’Ã‚Â¡pido e preÃƒÆ’Ã‚Â§o promocional.', brand: 'Samsung', deviceType: 'old' },
    { id: 'motorola_antigo_frp', title: 'Motorola Antigo - Conta Google (FRP)', price: 'R$ 20,00', description: 'RemoÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o de Conta Google (FRP) em modelos Motorola mais antigos (Moto G/E).', brand: 'Motorola', deviceType: 'old' },
    { id: 'lg_antigo_frp', title: 'LG Antigo - Conta Google (FRP)', price: 'R$ 20,00', description: 'RemoÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o de Conta Google (FRP) em modelos LG antigos.', brand: 'LG', deviceType: 'old' },
  ]);

  const [newDevices] = useState([
    { id: 'samsung_moderno_v2', title: 'Samsung - PayJoy/Google (Moderno)', price: 'R$ 300,00', description: 'Desbloqueio de bloqueio PayJoy e remoÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o de Conta Google (FRP) em modelos Samsung mais recentes (incluindo Android 14/15). PreÃƒÆ’Ã‚Â§o fixo: R$ 300,00.', brand: 'Samsung', deviceType: 'new' },
    ...
    { id: 'tecno_infinix_moderno', title: 'Tecno / Infinix - Bloqueios Diversos', price: 'R$ 180,00', description: 'ServiÃƒÆ’Ã‚Â§os de desbloqueio para Tecno e Infinix. PreÃƒÆ’Ã‚Â§os variÃƒÆ’Ã‚Â¡veis: R$ 180,00 (bloqueio complexo) / R$ 100,00 (bloqueio simples). Consulte para confirmar o valor exato.', brand: 'Tecno', deviceType: 'new' },
    ...
    { id: 'realme_c71_etc', title: 'Realme C71, N50, C63, C61 - PayJoy/Google', price: 'A partir de R$ 100,00', description: 'Modelos C71, Note 50, C63, C61 (PreÃƒÆ’Ã‚Â§os fixos). **PayJoy: R$ 180,00**. **Conta Google (FRP): R$ 100,00**.', brand: 'Realme', deviceType: 'new' },
  ]);
  ...
  const openWhatsApp = (serviceName, price) => {
    ...
    const message = `OlÃƒÆ’Ã‚Â¡! Gostaria de fazer o serviÃƒÆ’Ã‚Â§o: ${serviceName} por ${price}. Podem me chamar?`;
...
<<<<<<< HEAD
            PayJoy  Conta Google  Senhas <br /> ServiÃƒÆ’Ã‚Â§o rÃƒÆ’Ã‚Â¡pido e garantido.
=======
            PayJoy Ã¢â‚¬Â¢ Conta Google Ã¢â‚¬Â¢ Senhas <br /> ServiÃƒÂ§o rÃƒÂ¡pido e garantido.
>>>>>>> c35ef5e176dd9be1eba798a3e96c50140439fc1b
          ...
          <div className="space-y-4 w-full max-w-sm">
            ...
            >
              <Search size={20} /> <span>Verificar Modelo e PreÃƒÆ’Ã‚Â§o</span>
            </button>
            <button
              onClick={() => openWhatsApp('Melhores PreÃƒÆ’Ã‚Â§os/Consultas', 'Combinar')}
            ...
            >
              <span>Para melhores preÃƒÆ’Ã‚Â§os, toque aqui!</span>
            </button>
          </div>
        </div>
      );
            ...
            <button onClick={() => setFlowStep('list-old')} className="w-full bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex items-center justify-between group active:scale-[0.98] transition-all">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm text-gray-500">Modelos mais simples e econÃƒÆ’Ã‚Â´micos</p>
                </div>
              </div>
            ...
            <button onClick={() => setFlowStep('list-new')} className="w-full bg-white p-6 rounded-2xl shadow-lg border border-gray-100 flex items-center justify-between group active:scale-[0.98] transition-all">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <p className="text-sm text-gray-500">LanÃƒÆ’Ã‚Â§amentos recentes, com preÃƒÆ’Ã‚Â§os fixos</p>
                </div>
              </div>
              <ChevronRight className="text-gray-400 group-hover:text-blue-500" />
            </button>
      ...
      <div className="p-4 pb-24 animate-slide-left">
        ...
        {flowStep === 'list-old' && (
          <div className="bg-green-50 text-green-700 p-3 rounded-xl mb-5 text-sm font-medium text-center border border-green-200 shadow-sm">
            Lista de aparelhos mais antigos com preÃƒÆ’Ã‚Â§os promocionais.
          </div>
        )}
        {flowStep === 'list-new' && (
          <div className="bg-blue-50 text-blue-700 p-3 rounded-xl mb-5 text-sm font-medium text-center border border-blue-200 shadow-sm">
            Muitos preÃƒÆ’Ã‚Â§os sÃƒÆ’Ã‚Â£o fixos. Clique para ver detalhes e entrar em contato.
          </div>
        )}

        {/* Frase visÃƒÂ­vel imediatamente */}
        <div className="bg-yellow-100 text-yellow-800 p-3 rounded-xl mb-5 text-sm font-medium text-center border border-yellow-300 shadow-sm">
          Ã°Å¸â€˜â€° Deslize para cima para ver o botÃƒÂ£o de contato.
        </div>
  ...
  const ContactScreen = () => (
    <div className="p-6 animate-fade-in flex flex-col h-full justify-center">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-500 mt-2">DÃƒÆ’Ã‚Âºvidas? OrÃƒÆ’Ã‚Â§amentos especiais? <br /> Estou online para te atender.</p>
      </div>
      <button
        onClick={() => openWhatsApp('Atendimento Geral', 'OrÃƒÆ’Ã‚Â§amento')}
    ...
    return (
      <div className="p-6 animate-fade-in flex flex-col h-full justify-center text-center">
        <div className="text-center mb-10">
          ...
          <h2 className="text-2xl font-bold text-gray-800">ConheÃƒÆ’Ã‚Â§a Meu Trabalho</h2>
          <p className="text-gray-500 mt-2">Visite meu canal no YouTube para ver vÃƒÆ’Ã‚Â­deos dos serviÃƒÆ’Ã‚Â§os realizados e garantir sua confianÃƒÆ’Ã‚Â§a.</p>
        </div>
        <a
          href={youtubeUrl}
          target="_blank"
            ...
            <div className="bg-white w-full sm:w-11/12 sm:max-w-md sm:rounded-2xl rounded-t-3xl p-6 shadow-2xl animate-slide-up">
              ...
              <div className="border-t border-gray-100 pt-6">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-sm text-gray-500 uppercase font-bold">Valor do ServiÃƒÆ’Ã‚Â§o</span>
                ...
                >
                  ...
                  <span>{selectedService.price.includes('Consultar') || selectedService.price.includes('Chamar') ? 'Solicitar OrÃƒÆ’Ã‚Â§amento' : 'Fazer esse desbloqueio'}</span>
                </button>
                <p className="text-center text-xs text-gray-400 mt-3">VocÃƒÆ’Ã‚Âª serÃƒÆ’Ã‚Â¡ redirecionado para o WhatsApp</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white border-t border-gray-200 px-4 py-3 pb-safe sticky bottom-0 z-40">
          <div className="flex justify-around items-center">
            ...
            >
              ...
              <span className="text-xs font-medium">InÃƒÆ’Ã‚Â­cio</span>
            </button>
            <button
            ...
            >
              ...
              <span className="text-xs font-medium">VÃƒÆ’Ã‚Â­deos dos meus serviÃƒÆ’Ã‚Â§os</span>
            </button>
            <button
